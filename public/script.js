...
function initMainParticles() {
  ...
  Object.entries(counts).forEach(([type, count]) => {
    for (let i = 0; i < count; i++) {
      ...
      p.y = Math.random() * canvas.height; // start anywhere
...
// ══════════════ COUNTDOWN ══════════════
const weddingDate = new Date('2026-02-15T07:00:00+05:30');
...
function launchModalCelebration() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    span.style.cssText = `
      ...
      top: 100%;
      animation: modalFloat ${Math.random() * 2 + 1.5}s ease-out forwards;
  ...
  style.textContent = `
    @keyframes modalFloat {
      ...
      100% { transform: translateY(-${Math.random() * 200 + 200}px) rotate(${Math.random() * 360}deg); opacity: 0; }
    }
  `;
...
// ══════════════ DIYA PARTICLES ══════════════
// Extra ember particles around diyas
